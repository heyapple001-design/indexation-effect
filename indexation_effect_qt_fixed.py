...
class ColumnDialog(QDialog):
    ...
    ВАЖНО: в Excel заголовки колонок не всегда строки (могут быть числа, даты, NaN).
    Старый вариант передавал list(df.columns) напрямую в QListWidget.addItems()
    и падал с TypeError, если хотя бы один заголовок был не str.
    Здесь для интерфейса используются безопасные строковые подписи, а в self.selected
    возвращается исходное имя колонки, чтобы pandas-доступ df[self.selected] не ломался.
    """
    def __init__(self, columns: list[Any], title: str, instruction: str, parent=None):
        super().__init__(parent)
        ...
        self._all_items: list[tuple[str, Any]] = self._make_display_items(columns)
        self._display_to_column: dict[str, Any] = {display: original for display, original in self._all_items}
        self._list.addItems([display for display, _ in self._all_items])
    ...
    @staticmethod
    def _make_display_items(columns: list[Any]) -> list[tuple[str, Any]]:
        items: list[tuple[str, Any]] = []
        seen: dict[str, int] = {}
        for idx, col in enumerate(columns):
            display = "" if col is None else str(col)
            if display.strip() == "" or display.lower() == "nan":
                display = f"(пустой заголовок #{idx + 1})"
            base_display = display
            seen[base_display] = seen.get(base_display, 0) + 1
            if seen[base_display] > 1:
                display = f"{base_display}  [дубль {seen[base_display]}]"
            items.append((display, col))
        return items

    def _filter(self, text: str):
        self._list.clear()
        q = (text or "").lower()
        self._list.addItems([display for display, _ in self._all_items if q in display.lower()])
    ...
    def _pick(self, *args):
        ...
        if not items:
            ...
            return
        display = items[0].text()
        self.selected = self._display_to_column.get(display, display)
        self.accept()
...
class ClickableFrame(QFrame):
    """Безопасная кликабельная карточка для выбора режима.

    Старый код подменял mousePressEvent у конкретного QFrame через lambda.
    В PyQt это иногда приводит к нестабильному поведению после закрытия диалога.
    Отдельный класс с сигналом clicked работает штатно и не меняет бизнес-логику.
    """
    clicked = pyqtSignal()
...
class ChargesModeDialog(QDialog):
    def __init__(self, base_name: str, mrf_name: str, parent=None):
        ...
        def card(icon: str, title: str, desc: str, mode: str) -> QFrame:
            ...
            fl.addWidget(d)
            f.clicked.connect(lambda m=mode: self._choose(m))
            return f
    ...
    def _run(self):
        ...
        try:
            ...
            show_error("Критическая ошибка",
                       "Программа перехватила ошибку и не будет закрыта.\n\n"
                       f"{e}\n\nПодробности записаны в окно журнала.")
        finally:
...
def _validate_before_calc(win, df_base, df_amount, base_col, id_col, amount_col, current_col, charges_mode) -> bool:
    """Проверяет выбранные пользователем колонки до расчёта.

    Это защита от падений при неполном выборе режима/колонок или при файлах
    с неожиданной структурой. Вместо KeyError/TypeError показываем понятное сообщение
    и пропускаем текущую базу.
    """
    errors: list[str] = []
    if base_col is None or base_col not in df_base.columns:
        errors.append("не выбрана или не найдена колонка начисления ДО")
    if charges_mode == MODE_EXTERNAL:
        if df_amount is None:
            errors.append("не загружен файл начислений")
        if id_col is None or id_col not in df_base.columns:
            errors.append("не выбрана или не найдена колонка идентификатора в базе")
        if df_amount is not None:
            if "MSISDN" not in df_amount.columns:
                errors.append("в файле начислений нет колонки MSISDN")
            if amount_col not in df_amount.columns:
                errors.append(f"в файле начислений нет колонки '{amount_col}'")
    elif charges_mode == MODE_INBASE:
        if current_col is None or current_col not in df_base.columns:
            errors.append("не выбрана или не найдена колонка текущего начисления в базе")
    else:
        errors.append("не выбран режим источника начислений")

    if errors:
        msg = "Нельзя выполнить расчёт по текущей базе:\n" + "\n".join(f"  • {e}" for e in errors)
        win.log("  ❌ " + msg.replace("\n", "\n  "))
        show_error("Проверка данных", msg)
        return False
    return True
...
def process_one_base(win, df_base, df_amount, df_report,
    ...
    if not _validate_before_calc(win, df_base, df_amount, base_col, id_col, amount_col, current_col, charges_mode):
        return None
    ...
    try:
        ...
        tb = traceback.format_exc()
        win.log("   ❌ Ошибка расчёта:\n" + tb)
        show_error("Ошибка расчёта",
                   "База будет пропущена, программа продолжит работу.\n\n"
                   f"{e}")
        return None
    finally:
        wait.close_wait()
...
def _process_mrf(win, mrf_name, df_report, table3_path, month,
    ...
    while True:
        ...
        try:
            ...
            tb = traceback.format_exc()
            win.log("  ❌ Ошибка обработки базы:\n" + tb)
            show_error("Ошибка обработки базы",
                       "Текущая база будет пропущена, программа продолжит работу.\n\n"
                       f"{e}")
            res = None
...
def _qt_excepthook(exc_type, exc_value, exc_tb):
    """Последний рубеж защиты: необработанные исключения не должны молча ронять UI."""
    tb = "".join(traceback.format_exception(exc_type, exc_value, exc_tb))
    print(tb, file=sys.stderr)
    try:
        show_error("Необработанная ошибка",
                   "Возникла ошибка, но приложение продолжает работу.\n\n"
                   f"{exc_value}\n\nПодробности напечатаны в консоль/лог.")
    except Exception:
        pass
